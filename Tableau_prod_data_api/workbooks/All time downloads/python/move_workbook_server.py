...
def get_project(server,auth_token, site_id):
    ...
    for project in xml_response.findall('.//t:project', namespaces=xmlns):
        if project.get('name') != 'default':
            if not project.get('parentProjectId')
                parent_project.update({project.get('name'):project.get('description')})
            else:
                child_project.update({project.get('id'):{'name': project.get('name'),'parentProjectId':project.get('parentProjectId')}})
    return out`
...
def main():
    ...
    source_server =  "https://prod-uk-a.online.tableau.com" #raw_input("\n: source_server address")
    source_username = "exporting_token" #raw_input("\n source token key: ")
    workbook_name = "Workbook" #raw_input("\nName of workbook to move: ")
    dest_server = "https://tableau-staging03.corp.kwalee.com" #raw_input("\nDestination server: ")
    dest_username = "api" #raw_input("\nDestination server token key: ")
    source_site = "kwaleereporting"
    dest_site = ""

    print(f"\n*Moving Workbook from {source_server} to {dest_server} within same porject.")
    source_password = "U3ZL2w0lSVmDYd/KuzPPzw==:vKbqklN6W7kt7nbYzkXV4MAIQzyFtzaj" #getpass.getpass("Token for {0} on {1}: ".format(source_username, source_server))
    dest_password = "9mEQwj5lTb+hSlWoJpNQBQ==:c6hFNgd5hpTYOESZM3WgWJT7EnFPYXrI" #getpass.getpass("Token for {0} on {1}: ".format(dest_username, dest_server))
    #
    ##### STEP 1: Sign in #####
    ...
    for workbook_id, projectname in workbooks_project.items():
        ...
        ##### STEP 9: Deleting workbook from the source site #####
        # print(f"Do you want to delete this workbook from the {source_server} ? : Y (yes) / N (no)", end=":")
        # user_input = input()
        # if user_input.lower() != "y":
        #     continue
        # else:
        #     print(f"\n6. Deleting workbook from the original site {source_server} and temp file")
